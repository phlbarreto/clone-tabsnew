...
describe("PATCH /api/v1/activations/[token_id]", () => {
  describe("Anonymous user", () => {
    test("With nonexistent token", async () => {
      const response = await fetch(
        "http://localhost:3000/api/v1/activations/dde1a766-1dd0-4546-90b1-9d8ddc246009",
        {
          method: "PATCH",
        },
      );
    ...
    test("With expired token", async () => {
      jest.useFakeTimers({
        now: new Date(
          Date.now() - activation.EXPIRATION_IN_MILLISECONDS - 10000,
        ),
      });

      const createdUser = await orchestrator.createUser();
      const expiredActivationTokenObject = await activation.create(
        createdUser.id,
      );
      ...
      const response = await fetch(
        `http://localhost:3000/api/v1/activations/${expiredActivationTokenObject.id}`,
        {
          method: "PATCH",
        },
      );
  ...
  describe("Default user", () => {
    test("With valid token, but already logged in user", async () => {
      ...
      const user2ActivationTokenObject = await activation.create(user2.id);

      const response = await fetch(
        `http://localhost:3000/api/v1/activations/${user2ActivationTokenObject.id}`,
        {
          method: "PATCH",
          headers: {
